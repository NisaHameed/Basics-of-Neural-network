...
class BatchNorm1d:
    ...
    def forward(self, Z, eval=False):
        ...
        self.Z = Z
        self.N = Z.shape[0]
        self.M = self.Z.sum(axis=0)/self.N
        self.V = (((self.Z - self.M)**2).sum(axis=0))/self.N

        if eval == False:
            # training mode
            self.NZ = np.subtract(self.Z,self.M)/np.sqrt(self.V+self.eps)
    ...
    def backward(self, dLdBZ):
        ...
        t1 = -(2/self.N)*np.sum((self.Z -self.M),axis=0)
        t2 = (self.V + self.eps)**(-1.5) * t1
        dNZdM = -(self.V+self.eps)**(-0.5)-0.5 * (self.Z - self.M)*t2
